  ...
  <main class="relative fc h-100 ">
    <section class="relative fc h-90 box-orange streambox">
      <div class="frw h-100 ac-start" v-if="datasetActive && datasetActive.items.length">
        <div class="stream" v-for="(info,i) in datasetActive.items" v-bind:key="info.url">
          <Stream v-bind:streamInfo="info" v-bind:dataset="datasetActive"></Stream>
        </div>
      </div>
      <div v-else>
        choose a dataset...
    ...
    <footer class="fr h-10 box-orange">
      ...
      <ul class="fr ai-start jc-start">
        <li class="fr ai-start p-3 m-8 dataset" v-for="(datasetInfo,i) in datasetsInfo" v-bind:key="i">
          <span @click="onClickDataset(datasetInfo.name)">{{datasetInfo.name}}</span>
        </li>
      </ul>
...
const Stream = require('./hls.vue').default
...
export default class ViewStreamsHls extends Vue {
