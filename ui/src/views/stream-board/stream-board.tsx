...
import { StreamStream } from 'src/parts/stream'
      ...
      observer(
        class extends React.Component<IProps & WithStyles<"streams">, IState> {
          ...
          render() {
            ...
            const { datasetSelectedFilename, streams } = store
            const dataset = store.datasets.get(datasetSelectedFilename) || []
            const streamArr = streams.streams || []
                    ...
                    <Grid container direction={'row'} item xs={12} spacing={24} >
                      {
                        streamArr
                          .map((s,i) => {
                            // console.warn(s)
                            const info = dataset.find(x => x.version.object.cam_url == s.ffmpeg.url_src)
                            return (
                              <StreamStream key={`${s.ffmpeg.url_src}_${i}`} streamState={s}  streamInfo={info}/>
                            )
                          })
                      }
                    </Grid>
